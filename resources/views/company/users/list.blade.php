...
<div class="intro-y flex flex-col sm:flex-row items-center mt-8">
    ...
    <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
        <a href="{{route('company.usersAdd')}}" class="btn btn-primary shadow-md mr-2">Add New Users</a>
    </div>
</div>
                ...
                @if($users)
                    @foreach($users as $key=>$val)
                        <tr>
                            ...
                            <td class="whitespace-nowrap"><span class="alert-primary-soft p-2" style="border-radius: 10px">{{$val->role_id==3 ? 'Staff' : 'Technician'}}</span></td>
                            <td class="whitespace-nowrap">
                                <div class="flex lg:justify-center items-center">
                                    <a class="edit flex items-center mr-3" href="{{route('company.usersEdit',$val->id)}}">
                                    ...
                                    <a class="delete flex items-center text-danger" href="{{route('company.usersDelete',$val->id)}}" data-tw-toggle="modal" data-tw-target="#delete-modal-preview">
